...
- **collection_name** (Optional[str], optional): Name for the collection in the Milvus. If not provided, set it to the current time with iso format. (default: :obj:`None`) **kwargs (Any): Additional keyword arguments for initializing :obj:`MilvusClient`.
...
- **url_and_api_key** (Tuple[str, str]): The URL and API key for the Milvus server. **kwargs (Any): Additional keyword arguments passed to the Milvus client.
...
- **collection_name** (str): Name of the collection to be created. **kwargs (Any): Additional keyword arguments passed to create collection.
