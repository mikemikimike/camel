...
Deletes the collection if :obj:`delete_collection_on_del` is set
to :obj:`True`.
...
- **collection_name** (str): Name of the collection to be deleted. **kwargs (Any): Additional keyword arguments.
...
```python
def close_client(self, **kwargs: Any):
```
...
- **records** (List[VectorRecord]): List of vectors to be added. **kwargs (Any): Additional keyword arguments.
